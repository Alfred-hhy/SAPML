...
impl FieldChannel {
    fn init_from_path(&mut self, path: &str, id: usize) {
        ...
        for line in f.lines() {
            ...
            if trimmed.len() > 0 {
                let addr: SocketAddr = trimmed.parse().unwrap();
                addrs.push(addr);
            }
        }
    ...
    pub fn stream(&mut self) -> &mut TcpStream {
        self.stream.as_mut().expect("Unitialized FieldChannel. Did you forget init(..)?")
    }
...
pub fn init_from_path(path: &str, id: usize) {
    let mut ch = get_ch!();
    assert!(ch.stream.is_none(), "FieldChannel should no be re-intialized. Did you call init(..) twice?");
